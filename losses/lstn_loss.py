...
class LSTN_Loss(nn.Module):
    def __init__(self, lamda=0.001, beta=30):
        super(LSTN_Loss, self).__init__()
        self.lamda = lamda
        self.beta = beta
        
    def forward(self, preds_t0, preds_t1_blocks, gts, imgs):
        return lstn_loss(preds_t0, preds_t1_blocks, gts, imgs, self.lamda, self.beta)

class MLSTN_Loss(nn.Module):
    def __init__(self, lamda=0.001, beta=30):
        super(MLSTN_Loss, self).__init__()
        self.lamda = lamda
        self.beta = beta
        
    def forward(self, preds_t012, preds_t3, gts, imgs):
        return mlstn_loss(preds_t012, preds_t3, gts, imgs, self.lamda, self.beta)