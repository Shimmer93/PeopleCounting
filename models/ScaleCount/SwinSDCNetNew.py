...
class SwinSDCNetNew(nn.Module):
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            ...
            swin_2 = models.swin_s(weights=models.Swin_S_Weights.DEFAULT)
        else:
            swin = models.swin_s(weights=None)
            vgg = models.vgg16_bn(weights=None)
            swin_2 = models.swin_s(weights=None)

        features = list(swin.features.children())
        features_2 = list(swin_2.features.children())
        ...
        self.feat2_2 = nn.Sequential(*features_2[0:2])
        self.feat3_2 = nn.Sequential(*features_2[2:4])
        self.feat4_2 = nn.Sequential(*features_2[4:6])
        self.feat5_2 = nn.Sequential(*features_2[6:8])
        ...
        self.dec4 = nn.Sequential(
            ConvBlock(768+384+384, 1536),
            ConvBlock(1536, 768)
        )

        self.dec3 = nn.Sequential(
            ConvBlock(768+192, 768),
            ConvBlock(768, 384)
        )

        self.dec2 = nn.Sequential(
            ConvBlock(384+96, 384),
            ConvBlock(384, 192)
        )
        ...
        self.shuf5 = PixelShuffle_ICNR(768, 384, act_cls=nn.ReLU)
        ...
        self.scale_dec = nn.Sequential(
            ConvBlock(768+192, 768, padding=2, dilation=2),
    ...
    def _forward(self, x):
        ...
        x1_2 = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=True)
        x2_2 = self.feat2_2(x1_2) # 96
        x3_2 = self.feat3_2(x2_2) # 192
        x4_2 = self.feat4_2(x3_2) # 384
        x5_2 = self.feat5_2(x4_2) # 768
        ...
        x3_2 = x3_2.permute(0, 3, 1, 2)
        x4_2 = x4_2.permute(0, 3, 1, 2)
        x5_2 = x5_2.permute(0, 3, 1, 2)

        x1_vgg = self.feat_vgg(x)

        d5 = self.dec5(x5_2) # 768
        x = self.shuf5(d5) # 384

        x = torch.cat([x, x4, x4_2], dim=1) # 384+768+384
        d4 = self.dec4(x) # 768
        x = self.shuf4(d4) # 384

        x = torch.cat([x, x3, x3_2], dim=1) # 384+384+192
        x_scale = x
        d3 = self.dec3(x) # 384
        o3 = d3 
        x = self.shuf3(d3) # 192

        x = torch.cat([x, x2, x2_2], dim=1) # 192+192+96
        d2 = self.dec2(x) # 192
        o2 = d2
        x = self.shuf2(d2) # 96

        x = torch.cat([x, x1, x1_vgg], dim=1) # 96+96+256
        d1 = self.dec1(x) # 96
        o1 = d1
