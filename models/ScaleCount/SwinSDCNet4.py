...
class SwinSDCNet4(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.dec4 = nn.Sequential(
            ConvBlock(768+768, 1536),
            ConvBlock(1536, 768)
        )

        self.dec3 = nn.Sequential(
            ConvBlock(768+384, 768),
            ConvBlock(768, 384)
        )

        self.dec2 = nn.Sequential(
            ConvBlock(384+192, 384),
            ConvBlock(384, 192)
        )

        self.dec1 = nn.Sequential(
            ConvBlock(192+256+96, 192),
            ConvBlock(192, 96)
        )
        ...
        self.scale_dec = nn.Sequential(
            ConvBlock(768+384, 768, padding=2, dilation=2),
        ...
        self.shallow_enc1 = nn.Sequential(
            ConvBlock(3, 96, kernel_size=3, padding=1),
            ConvBlock(96, 96, kernel_size=3, padding=1),
            ConvBlock(96, 96, kernel_size=3, padding=1)
        )

        self.shallow_enc2 = nn.Sequential(
            ConvBlock(3, 96, kernel_size=3, padding=1),
        ...
        self.shallow_enc3 = nn.Sequential(
            ConvBlock(3, 96, kernel_size=3, padding=1),
            ConvBlock(96, 192, kernel_size=3, padding=1),
            ConvBlock(192, 384, kernel_size=3, padding=1),
            ConvBlock(384, 384, kernel_size=3, padding=1),
            ConvBlock(384, 384, kernel_size=3, padding=1)
        )

        self.shallow_enc4 = nn.Sequential(
            ConvBlock(3, 96, kernel_size=3, padding=1),
            ConvBlock(96, 192, kernel_size=3, padding=1),
            ConvBlock(192, 384, kernel_size=3, padding=1),
            ConvBlock(384, 768, kernel_size=3, padding=1),
            ConvBlock(768, 768, kernel_size=3, padding=1),
            ConvBlock(768, 768, kernel_size=3, padding=1)
        )

    def _forward(self, x):
        x1_shallow = F.interpolate(x, scale_factor=1/4, mode='bilinear', align_corners=True)
        x2_shallow = F.interpolate(x, scale_factor=1/8, mode='bilinear', align_corners=True)
        x3_shallow = F.interpolate(x, scale_factor=1/16, mode='bilinear', align_corners=True)
        x4_shallow = F.interpolate(x, scale_factor=1/32, mode='bilinear', align_corners=True)

        x1_shallow = self.shallow_enc1(x1_shallow)
        x2_shallow = self.shallow_enc2(x2_shallow)
        x3_shallow = self.shallow_enc3(x3_shallow)
        x4_shallow = self.shallow_enc4(x4_shallow)

        print(x1_shallow.max(), x2_shallow.max(), x3_shallow.max(), x4_shallow.max())
        plt.imshow(x1_shallow[0, 0].detach().cpu().numpy())
        plt.savefig(f'x1_shallow_{time}.png')
        ...
        x = torch.cat([x4, x4_shallow], dim=1) # 1536
        d4 = self.dec4(x) # 768
        x = self.shuf4(d4) # 384

        x = torch.cat([x, x3, x3_shallow], dim=1) # 768
        x_scale = x
        d3 = self.dec3(x) # 384
        o3 = d3 
        x = self.shuf3(d3) # 192

        x = torch.cat([x, x2, x2_shallow], dim=1) # 384
        d2 = self.dec2(x) # 192
        o2 = d2
        x = self.shuf2(d2) # 96

        x = torch.cat([x, x1, x1_vgg, x1_shallow], dim=1) # 192
        d1 = self.dec1(x) # 96
        o1 = d1
